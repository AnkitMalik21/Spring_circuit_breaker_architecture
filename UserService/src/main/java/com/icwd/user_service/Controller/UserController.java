...
Because you have max-attempts: 3 in your Retry configuration,
 ...
 Spring Boot internally attempts to execute the getUserById method 3 times (1 initial try + 2 retries).

Since the @CircuitBreaker is wrapped around the
 @Retry (thanks to the aspect-order properties),
 the Circuit Breaker only evaluates the result after all 3 retries have finished and failed. Therefore, to the Circuit Breaker, those 3 internal attempts only count as 1 call.

Because your Circuit Breaker requires minimum-number-of-calls: 5:

You must make at least 5 separate HTTP requests that fail.
5 HTTP requests × 3 attempts each = 15 total entries into your method logic.
 So yes, your retryCount log will reach 15 before the Circuit Breaker even begins calculating the failure rate (and subsequently trips OPEN).

Try making those 5 requests (so that the retryCount hits 15), and you will see the Circuit Breaker open up!
 */
